...
ALTER TABLE `user_server_manage` ADD CONSTRAINT `user_server_manage_serverId_server_id_fk` FOREIGN KEY (`serverId`) REFERENCES `server`(`id`) ON DELETE no action ON UPDATE no action;