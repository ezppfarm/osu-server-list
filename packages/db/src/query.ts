...
export const getAllServersWithHooks = async (): Promise<ServerFullHook[]> => {
  ...
  return await db
    .select({
      ...
      date_added: server.date_added,
      location: server.location,
      postbackUrl: stringWithFallback(serverVoteHook.postback_url, "").as(
        "postbackUrl",
      ),
      discordWebhookUrl: stringWithFallback(
        serverVoteHook.discord_webhook_url,
        "",
      ).as("discordWebhookUrl"),
      discordWebhookContent: stringWithFallback(
        serverVoteHook.discord_webhook_content,
        "",
      ).as("discordWebhookContent"),
    })
    .from(server)
    .leftJoin(latestStatus, eq(server.id, latestStatus.serverId))
    .leftJoin(
    ...
    .leftJoin(serverVote, eq(server.id, serverVote.serverId))
    .leftJoin(serverVoteHook, eq(server.id, serverVoteHook.server_id))
    .groupBy(
      server.id,
      server.name,
      server.type,
